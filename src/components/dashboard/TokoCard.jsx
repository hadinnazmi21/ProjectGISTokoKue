  ...
  return (
    <div className="bg-white rounded-xl border-2 border-slate-200 hover:border-rose-300 hover:shadow-xl transition-all duration-300 overflow-hidden group">
      ...
      <div className="relative h-48 bg-slate-100 overflow-hidden">
        {toko.gambar ? (
          <img
            ...
            onError={(e) => {
              ...
              const icon = getProductIcon(toko.produk || 'Kue');
              e.target.parentElement.innerHTML = `<div class="w-full h-full flex items-center justify-center text-6xl bg-gradient-to-br from-rose-100 to-pink-100">${icon}</div>`;
            }}
          />
        ) : (
          <div className="w-full h-full flex items-center justify-center text-6xl bg-gradient-to-br from-rose-100 to-pink-100">
            {getProductIcon(toko.produk || 'Kue')}
          </div>
        )}
        ...
        {/* Produk Badge */}
        <div className="absolute top-3 left-3 px-3 py-1 bg-gradient-to-r from-rose-500 to-pink-500 text-white rounded-full shadow-lg flex items-center gap-1">
          <span className="text-sm">{getProductIcon(toko.produk || 'Kue')}</span>
          <span className="text-xs font-bold">{toko.produk || 'Kue'}</span>
        </div>
      </div>
      ...
      <div className="p-5">
        ...
        {/* Action Buttons - Only show if onEdit and onDelete are provided */}
        {onEdit && onDelete && (
